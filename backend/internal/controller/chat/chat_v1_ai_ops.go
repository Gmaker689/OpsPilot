...
func (c *ControllerV1) AIOps(ctx *gin.Context) {
	var req v1.AIOpsReq
	// body 为空时 ShouldBindJSON 会报 EOF，但 AIOps 不依赖请求参数，忽略即可
...
`
	...
	resp, detail, err := plan_execute_replan.BuildPlanAgent(ctx.Request.Context(), query)
	if err != nil {
		ctx.Error(err)
		return
	}
	if resp == "" {
		ctx.Error(errors.New("内部错误"))
		return
	}

	ctx.Set("response", &v1.AIOpsRes{
		Result: resp,
		Detail: detail,
	})
}
